...
def _replace_payload(temporary: Path, path: Path, previous: bytes | None) -> None:
    ...
    while True:
        try:
            ...
            guard = _RETRY_GUARD.get()
            if guard is None or not _replace_contended(error, path):
                raise
            ...
            guard()
            # Keep the initial CAS result; never overwrite another writer's
            # intervening fact. The caller can reload/reconcile the original event.
            current = path.read_bytes() if path.exists() else None
            if current != previous:
                raise StateEvolutionError('state changed during atomic replacement wait') from None
...
class JsonSubjectStateRepository:
    ...
    def _write_payload(self, path: Path, data: dict[str, Any]) -> None:
        ...
        try:
            ...
            _replace_payload(temporary_path, path, previous)
