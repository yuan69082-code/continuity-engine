"""One durable P18 repair test invocation; original outputs survive Fixture cleanup."""
import contextlib
import datetime
import hashlib
import json
    ...
    with (DIRECTORY / (label + '.stdout.log')).open('x', encoding='utf-8') as stdout, \
        ...
        with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
            try:
                ...
                if prefixes and prefixes[0] == '--review-class':
                    sys.path[:0]=[str(DIRECTORY/'independent'),str(ROOT/'tests')]
                    import importlib
                    module_name=prefixes[1].split('.')[0]
                    if module_name=='test_independent_boundaries':
                        module=importlib.import_module(module_name)
                        observed=DIRECTORY/(label+'-observations')
                        observed.mkdir()
                        module.OUT=observed
                    discovered=list(flatten(loader.loadTestsFromName(prefixes[1])))
                    prefixes=[]
                elif prefixes == ['--independent-probe']:
                    import importlib.util
                    spec=importlib.util.spec_from_file_location('test_independent_runtime',DIRECTORY/'independent/test_independent_runtime.py')
