...
class ThinkingService:
    ...
    def handle_perception(
        ...
        *,
        ...
        track_provider_execution: bool = False,
    ) -> ThinkingExecutionResult:
        ...
        if input_validator is not None:
            input_validator(perception.to_dict())
        if execution_guard is not None:
            execution_guard('before_thinking_resources')
        ...
        try:
            ...
            from continuity_engine.domain.persistent_runtime import RuntimeDeferred
            if track_provider_execution and isinstance(exc, RuntimeDeferred):
                # No ThinkSession or provider call exists at this point. Preserve
                # original control/usage facts without manufacturing FAILED.
                raise
        ...
        if execution_guard is not None:
            execution_guard('after_thinking_resources')
        session = ThinkSession.start(
            wake_session_id=perception.wake_session_id,
            subject_id=perception.subject_id,
            provider_id=self._provider.provider_id,
        ...
        if track_provider_execution and budget.session_tokens:
            session.record_provider_phase('PREPARED', created_at, 'PREPARED')
        self._repository.save_think_session(session)
        ...
        return self._run_provider(session, perception, result_id=result_id,
            result_processor=result_processor, result_validator=result_validator,
            execution_guard=execution_guard)

    def resume_prepared_provider(self, session, *, result_id, result_processor,
                                 result_validator, execution_guard, input_validator):
        """Continue only positively bound native pre-call work, never old FAILED."""
        current = self.get_session(session.subject_id, session.think_id)
        if current.to_dict() != session.to_dict() or not current.provider_not_started:
            raise ThinkingValidationError('THINKING_PROVIDER_RESUME_BINDING')
        perception = current.perception_snapshot
        input_validator(perception.to_dict())
        execution_guard('before_thinking_resources')
        self._subject_states.require_active(current.subject_id)
        budget = self._token_budgets.allocate(TokenBudgetRequest(
            current.subject_id, current.wake_session_id, current.token_budget.depth,
            self._clock(), session_id=current.think_id, model_name=current.provider_id,
            reason=current.thinking_reason))
        if budget.session_tokens != current.token_budget.session_tokens:
            raise ThinkingValidationError('THINKING_PROVIDER_RESERVATION_BINDING')
        execution_guard('after_thinking_resources')
        return self._run_provider(current, perception, result_id=result_id,
            result_processor=result_processor, result_validator=result_validator,
            execution_guard=execution_guard)

    def abandon_prepared_provider(self, session):
        current = self.get_session(session.subject_id, session.think_id)
        if current.to_dict() != session.to_dict() or not current.provider_not_started:
            raise ThinkingValidationError('THINKING_PROVIDER_RESUME_BINDING')
        current.fail(ended_at=self._clock(), result=self._failure_result(
            provider_id=current.provider_id, budget=current.token_budget,
            summary='Unexecuted native input requires current material reassessment.'),
            error='THINKING_MATERIAL_REASSESSMENT')
        current.record_provider_phase('ABANDONED', self._clock(), 'MATERIAL_REASSESSMENT')
        self._repository.save_think_session(current)

    def _run_provider(self, session, perception, *, result_id, result_processor,
                      result_validator, execution_guard):
        budget = session.token_budget
        material_error = None
        try:
            if execution_guard is not None:
                execution_guard('before_provider')
            if session.provider_execution is not None:
                self._token_budgets.validate_reservation(session)
                session.record_provider_phase('ENTERED', self._clock(), 'CALL_STARTED')
                self._repository.save_think_session(session)
            ...
            if session.provider_execution is not None:
                session.record_provider_phase('RETURNED', self._clock(), 'RESULT_VALIDATED')
            self._repository.save_think_session(session)
            return ThinkingExecutionResult(
                perception=perception,
                session=session,
            ...
            from continuity_engine.domain.persistent_runtime import RuntimeDeferred
            if session.provider_not_started and isinstance(exc, RuntimeDeferred):
                session.record_provider_phase('PREPARED', self._clock(), 'CONTROL_DEFERRED')
                self._repository.save_think_session(session)
                raise
            failure_result = self._failure_result(
                provider_id=self._provider.provider_id,
            ...
            if (session.provider_execution is not None
                    and session.provider_execution['events'][-1]['phase'] == 'PREPARED'):
                session.record_provider_phase('ABANDONED', self._clock(), 'MATERIAL_REASSESSMENT')
            self._repository.save_think_session(session)
