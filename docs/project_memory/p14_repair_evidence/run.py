"""One durable P14 test invocation; original outputs survive Fixture cleanup."""
import contextlib
import datetime
import hashlib
import json
    ...
    with (DIRECTORY / (label + '.stdout.log')).open('x', encoding='utf-8') as stdout, \
        ...
        with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
            try:
                ...
                if prefixes == ['--confirmed']:
                    sys.path.insert(0, 'C:\\Users\\Administrator\\Documents\\Codex\\2026-08-24\\https-github-com-yuan69082-code-continuity\\reviews\\p14-independent-review-20260908')
                    discovered = list(flatten(loader.loadTestsFromName('confirmed_probes')))
                    prefixes = []
                else:
