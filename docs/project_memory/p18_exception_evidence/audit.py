...
def git(*args):
    result=subprocess.run(['git','-c','core.quotepath=false',*args],cwd=ROOT,capture_output=True,encoding='utf8',env={**os.environ,'GIT_OPTIONAL_LOCKS':'0'})
    if '--check' not in args:result.check_returncode()
    elif result.returncode not in (0,1,2):result.check_returncode()
    return result
...
b=read(OUT/'before.json');frozen=read(OUT/'frozen-source.json')
source=runpy.run_path(str(OUT/'run.py'))['source_hashes']()
from continuity_engine.testing.persistence import tree_inventory_hash
assert source==frozen['source']
assert set(b['testIdentities']) <= set(frozen['identities'])
assert set(frozen['addedIdentities']) == set(frozen['identities']) - set(b['testIdentities'])
assert len(frozen['identities']) == len(set(frozen['identities']))
assert all(source.get(p)==h for p,h in b['sourceTest'].items() if p.startswith('tests/'))
assert {p for p,h in b['sourceTest'].items() if source.get(p)!=h} == set(b['allowedRuntimeFiles'])
...
assert sha(ROOT/'.git/index')==b['gitIndexHash']
...
full=read(OUT/'full-final-01.json')
assert set(full['testIdentities'])==set(frozen['identities'])
assert full['run']==len(frozen['identities'])
old_full=read(OUT.parent/'p18_storage_final_evidence/full-final-01.json')
assert full['skips']==old_full['skips']
observation=read(OUT/(sys.argv[2] if len(sys.argv)>2 else 'process-observation-final.json'))
...
for rel in pending:
    ...
    assert not parts & {'__pycache__','.pytest_cache','build','dist','node_modules','.continuity-data','.assistant-data'}
    assert Path(rel).suffix.lower() not in {'.pyc','.pyo','.whl','.zip','.tmp','.swp'}
assert all(p.startswith('docs/project_memory/p18_exception_evidence/') or p in docs or
           p in {'tests/test_p18_storage_exceptions.py'} for p in new)
...
gitstatus={'branch':'main','head':head,'localOrigin':origin,'aheadBehind':git('rev-list','--left-right','--count','HEAD...origin/main').stdout.strip(),
 ...
 'statusPorcelain':git('status','--porcelain=v1','--untracked-files=all').stdout,
 'statusPorcelainMoment':'Before this audit file is created; full untracked list includes it. Final read-only check follows.',
 'remoteQueried':False}
