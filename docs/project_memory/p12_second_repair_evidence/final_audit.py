...
probe_run=json.loads((directory/'original-final.json').read_text(encoding='utf-8'))
neighbor_run=json.loads((directory/'neighbors-final.json').read_text(encoding='utf-8'))
original=json.loads((root/'docs/project_memory/p12_repair_evidence/full-stable-02.tests.json').read_text(encoding='utf-8'))
...
runtime={'src/continuity_engine/'+x for x in ('storage/json_memory_repository.py','services/memory_service.py')}
test_path='tests/test_p12_memory_second_review.py'
...
docs={'README.md',*{'docs/project_memory/'+x for x in (
    ...
    '61_P12_生命周期恢复删除与传播语义.md','62_P12_测试索引与验收入口.md','P12_第二轮返修_F1-F2.md')}}
...
excluded=initial['p10Untracked']; owned=sorted((set(tracked+untracked)-set(excluded))|{
    'docs/project_memory/p12_second_repair_evidence/'+x for x in ('final-audit.json','pending-files.json','pending-files.md')})
repair_new=[p for p in untracked if p not in before['untracked']]
new_scope_errors=[p for p in repair_new if p not in test_paths and p not in docs and not p.startswith('docs/project_memory/p12_second_repair_evidence/')]
categories={
...
manifest={'head':git('rev-parse','HEAD')['stdout'],'branch':git('branch','--show-current')['stdout'],
    ...
    'repairAddedFiles':sorted(set(repair_new)|{str(out.relative_to(root)).replace('\\','/'),
        'docs/project_memory/p12_second_repair_evidence/pending-files.json','docs/project_memory/p12_second_repair_evidence/pending-files.md'}),
    'excludedP10':excluded,'gitWrites':False,'otherUnattributedFiles':new_scope_errors}
(directory/'pending-files.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
lines=['# P12 第二轮返修终局精确工作区清单','','只供独立复核，不构成暂存/提交授权。本轮没有 Git 写操作。',
    ...
    '初版和第一轮 pending-files.md/json 保留历史；本清单覆盖全部未提交 P12 成果。31 个原有 P10 脚本原样排除，逐文件 hash 见 JSON。',
    '本轮只修改两份运行文件，新增一份正式测试；原测试和原始证据不改。证据与辅助脚本须在未来提交授权时逐项判断，不能无差别暂存。','']
for name,paths in categories.items():
    lines+=['## '+name,'']+['- `'+p+'`' for p in paths]+['']
lines+=['## 本轮相对返修基线实际修改的已有文件','']+['- `'+p+'`' for p in drift]+['']
lines+=['## 其他无法归属的新增文件','']+['- `'+p+'`' for p in new_scope_errors]+([] if new_scope_errors else ['无。'])+['']
...
probe=next(Path(p) for p in review if p.endswith('/neighbor_probes.py') or p.endswith('\\neighbor_probes.py'))
def original_class(path):
    return next(x for x in ast.parse(path.read_text(encoding='utf-8')).body if isinstance(x,ast.ClassDef) and x.name=='P12ExistingAliasProbes')
...
source_matches=source==special['sourceTest']==compat['sourceTest']==full['sourceTest']==probe_run['sourceTest']==neighbor_run['sourceTest']
completed=(all(r['status']=='FINISHED' and not r['errors'] and not r['failures'] for r in (special,compat,full))
    ...
    and neighbor_run['exitCode']==0 and not neighbor_run['interrupted'])
retained=set(original['testIdentities']).issubset(full['testIdentities'])
new_ids=sorted(set(full['testIdentities'])-set(original['testIdentities']))
identity_errors=[x for x in new_ids if not x.startswith('test_p12_memory_second_review.')]
if len(new_ids)!=19: identity_errors.append('expected the 19 actual new test methods')
if len(full['testIdentities'])!=len(set(full['testIdentities'])): identity_errors.append('duplicate identity')
if full['skips']!=original['skips']: identity_errors.append('existing skip identity/reason drift')
state_errors=[]
for p in docs-{'docs/project_memory/P12_第二轮返修_F1-F2.md'}:
    text=(root/p).read_text(encoding='utf-8'); block=text.split('<!-- P12_SECOND_CURRENT_START -->')[-1].split('<!-- P12_SECOND_CURRENT_END -->')[0]
...
matrix=(root/'docs/project_memory/60_P12_规划施工测试验收矩阵.md').read_text(encoding='utf-8')
matrix_rows=re.findall(r'^\| P12-(\d{2}) \|[^\n]*\| IMPLEMENTED_NOT_ACCEPTED \|$',matrix,re.M)
if sorted(matrix_rows)!=[f'{i:02}' for i in range(1,13)]: state_errors.append('twelve current matrix rows')
decisions=(root/'docs/project_memory/04_决策记录.md').read_text(encoding='utf-8')
if re.search(r'^## D-061[：:]',decisions,re.M): state_errors.append('unexpected acceptance decision D-061')
...
report={'capturedAt':datetime.now(timezone.utc).isoformat(),'git':manifest,'sourceTest':source,
    'ownedContentHashes':{p:sha(root/p) for p in owned if (root/p).is_file() and (root/p)!=out},
    'allStableSourceMatches':source_matches,'originalTestsRetained':retained,'originalIdentityCount':len(original['testIdentities']),
    'currentIdentityCount':len(full['testIdentities']),'addedTestIdentities':new_ids,'identityErrors':identity_errors,'allExpectedRunsCompleted':completed,
    ...
    'matrixRows':matrix_rows,'acceptanceDecisionCreated':False,
...
errors=(not source_matches or not completed or not retained or not probe_preserved or protected!=before['protected']
    ...
    or identity_errors or ast_errors or broken or sensitive or new_cache or state_errors or diff['exitCode'] or version!='0.1.0'
