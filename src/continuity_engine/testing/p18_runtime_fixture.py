...
class P18Fixture(P17Fixture):
    ...
    def reopen(self):
        ...
        from .p18_persistence_diagnostics import install, exception_chain, emit, lock_owned
        self._persistence_context={}
        repositories={id(service._repository):service._repository for service in
                      (self.core.subject_states,original._subject_states)}
        for repository in repositories.values():
            install(repository,root=self.runtime.data_root,context=lambda:self._persistence_context)
        def diagnosed_continue(request):
            self._persistence_context={'task_id':request.task_id,'attempt_id':request.attempt_id}
            try:
                ...
                emit(dict(stage='native-exception-before-cleanup',task_id=request.task_id,
                    attempt_id=request.attempt_id,chain=chain,systemChain=exception_chain(exc),
                    stateWriteLockOwnedAfterUnwind=lock_owned()))
                raise
            finally:
                self._persistence_context={}
