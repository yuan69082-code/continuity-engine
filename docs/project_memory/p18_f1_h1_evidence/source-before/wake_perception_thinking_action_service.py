...
class WakePerceptionThinkingActionService:
    ...
    def __init__(
        ...
        *,
        ...
        self._runtime_fault = fault or (lambda stage: None)
    ...
    def _continue_mind(self, awakening, depth):
        ...
        This native opportunity has no PlatformObservation. P18 may explicitly
        connect the original C1 Action/E5-A dispatch; the P14 default is unchanged.
        """
        ...
        if session is None:
            if self._runtime_guard is not None:
                self._runtime_guard('before_native_perception')
            ...
            def initial_guard(stage):
                self._runtime_guard(stage)
                if stage == 'before_provider':
                    core.before_thinking(perception)
                    core.validate_input_material(perception.to_dict())
            thinking = self._thinking.handle_perception(perception, depth=depth, think_id=think_id,
                ...
                **({'execution_guard': initial_guard, 'input_validator': core.validate_input_material,
                    'result_validator': core.validate_thinking_material,
                    'track_provider_execution': True} if self._runtime_guard is not None else {}))
            session = thinking.session
            self._runtime_fault('after_native_thinking')
        else:
            perception = session.perception_snapshot
            if self._runtime_guard is not None and session.provider_not_started:
                core.before_thinking(perception)
                def resume_guard(stage):
                    self._runtime_guard(stage)
                    if stage == 'before_provider':
                        core.before_thinking(perception)
                        core.validate_input_material(perception.to_dict())
                thinking = self._thinking.resume_prepared_provider(session,
                    result_id=str(uuid5(NAMESPACE_URL, 'p14-result|' + wake.session_id)),
                    result_processor=core.process_thinking,
                    result_validator=core.validate_thinking_material,
                    execution_guard=resume_guard, input_validator=core.validate_input_material)
                session = thinking.session
        ...
        def native_action():
            return self._action.decide(ActionContext.from_results(
                subject_id=core.subject_id, subject_state_revision=perception.source_revision,
                thinking=thinking, perception=perception,
                current_time=perception.perceived_at if self._native_dispatch else self._clock(),
        ...
        core.before_thinking(perception)
        if self._runtime_guard is not None:self._runtime_guard('before_native_action')
