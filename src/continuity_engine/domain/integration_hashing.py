...
def calculate_state_hash(state: Mapping[str, Any] | JsonSerializable) -> str:
    """Hash the complete authoritative SubjectState representation."""

    value = _json_value(state)
    if not isinstance(value, dict):
        raise MachineContractValidationError("SubjectState hash input must be an object")
    return sha256_hash(canonicalize_json(value))


def calculate_projection_content_hash(
    snapshot: Mapping[str, Any] | JsonSerializable,
) -> str:
    """Hash the minimal first-round projection snapshot object."""

    value = _json_value(snapshot)
    if not isinstance(value, dict):
        raise MachineContractValidationError(
            "state projection snapshot hash input must be an object"
        )
    return sha256_hash(canonicalize_json(value))
