...
affected_prefixes=('test_p04_','test_p05_','test_p06_','test_p07_',
                   'test_p09_boundaries.','test_p09_continuity_core.',
                   'test_learning.','test_memory_service.','test_pre_p12_')
tests=[t for t in flatten(discovered) if scope=='full' or
       (scope=='p12' and t.id().startswith('test_p12_')) or
       (scope=='affected' and t.id().startswith(affected_prefixes)) or
