...
class SchedulerService:
    ...
    def __init__(
        ...
        *,
        ...
        subject_states=None,
        resource_scan_limit: int = 1,
    ) -> None:
        for value, name in (
            (capacity, "capacity"),
            (aging_interval_seconds, "aging_interval_seconds"),
            (retry_base_seconds, "retry_base_seconds"),
            (resource_scan_limit, "resource_scan_limit"),
        ):
            if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
                raise SchedulerValidationError(f"{name} must be positive")
        if resource_scan_limit > capacity:
            raise SchedulerValidationError("resource_scan_limit must not exceed capacity")
        for value, name in (
        ...
        self._resource_scan_limit = resource_scan_limit
    ...
    def tick(self, subject_id: str, environment: str) -> SchedulerTickResult:
        ...
        # Opt-in hosting may have work requiring different resources. Keep the
        # original priority order and one dispatch; a read-only rejected preview
        # does not spend an attempt, allocate resources or remove the task.
        # The default scans one candidate, preserving the original P11 path.
        deferred = None
        for task in ready[:min(self._resource_scan_limit, self._capacity)]:
            ...
            reason = "RESOURCE_POLICY_DEFERRED"
            try:
                resource_result = self._resources.preview(resource_request)
                allowed = resource_result.allowed and not resource_result.defer
            except Exception:
                allowed = False
                reason = "RESOURCE_STATE_UNAVAILABLE"
            if allowed:
                break
            if deferred is None:
                deferred = SchedulerTickResult(
                    SchedulerTickStatus.RESOURCE_DEFERRED,
                    task.clone(), queue.revision, reason,
                )
        else:
            return deferred
