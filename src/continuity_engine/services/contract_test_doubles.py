...
class ContractReplyComposer(Protocol):
    """Test-only subject-expression seam after Thinking and Action."""
...
class DeterministicMemoryRetriever:
    """Return one fixed relevant memory without external storage or network access."""
...
class DeterministicMemoryInfluenceRecorder:
    """In-process recorder used only to satisfy the MemoryService port."""
...
class DeterministicTokenBudgetManager:
    """Allocate a fixed non-billing budget for deterministic Thinking tests."""
...
class DeterministicThinkingProvider:
    """Read only PerceptionResult and produce the two fixed E3 thought outcomes."""
    ...
    def think(
        ...
        if not perception.external_facts:
            raise ValueError(
                "the first-round provider requires a perceived external fact"
            )
...
class DeterministicContractReplyComposer:
    """Compose a stable subject expression only from completed Thinking and Action."""
